<p align="center">
  <img src="https://raw.githubusercontent.com/Kaustav2706/Kaustav2706/main/profile.jpeg" width="180" alt="Kaustav Halder" />
</p>

<h1 align="center">Hi 👋, I'm Kaustav Halder</h1>
<h3 align="center">B.Tech CSE Student | Full Stack Developer | Open Source Enthusiast</h3>



🎓 B.Tech CSE Student at Manipal University Jaipur  
💻 Passionate about Full Stack Development, AI, and Open Source  
🚀 Interested in Hackathons, GSSoC, and building impactful projects

---

## 🛠 Tech Stack

**Languages:** Java, React, JavaScript, TypeScript, C++ , HTML , CSS 
**Frontend:** React, Next.js, Tailwind CSS  
**Backend:** Node.js, Express.js   
**Tools:** Git, GitHub, VS Code, Android Studio

---

## 🚀 Featured Projects

### AtomQuest
Enterprise Performance Management System with Employee, Manager, and Admin portals.

### FinVault
Finance tracker for budgeting and expense management.

### UltimateHealth
Health platform with AI chat, wellness content, and podcasts.

### Portfolio Website
Personal website showcasing projects and achievements.

### Med Manager
Medicine management application that helps users track prescriptions, set medication reminders, and monitor dosage schedules.

### Turf Booking System
Sports turf reservation platform where users can browse available slots, book grounds, and manage their bookings seamlessly.


---

## 🏆 Achievements

- Open Source Contributor
- 1x Hackathon Winner


---

## 📫 Connect With Me

- GitHub: https://github.com/Kaustav2706
- LinkedIn: https://www.linkedin.com/in/kaustav-halder-836904374/
- Email: kaustavhalder2706@gmail.com
- Portfolio : https://portfolio-web-one-dusky.vercel.app/
- Location : Kolkata, West Bengal

---

⭐ Thanks for visiting my profile!
